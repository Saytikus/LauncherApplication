...
void DataHandlerThreadPool::StartHandlerThreads() {
    int current_thread_count = 0;
    for(int i = 0; i < max_threads_count_; i++) {
        QThread *thread = new QThread();
        ++current_thread_count;
        thread->setObjectName("Handler thread №" + QString::number(current_thread_count));
...
void DataHandlerThreadPool::SendReadBufferChangeToHandler(const QBuffer *read_buffer, const QString buffer_id) {
    qDebug() << "Поток в ReadBufferChange: " << QThread::currentThread(); //
    qDebug() << "Данные read_buffer в ReadBufferChange: " << read_buffer->data();
    DataHandler *handler = handlers[roundrobin_count_ % max_threads_count_];
    ++roundrobin_count_;
...
void DataHandlerThreadPool::SetMaxThreadCount(const int thread_count) {
    max_threads_count_ = thread_count;
}
...
void DataHandlerThreadPool::TransferSendCompleteAnswer(const QByteArray complete_answer, const int size, const QString buffer_id) {
    ...
    emit CompleteAnswerFinished(complete_answer, size, buffer_id);
}
