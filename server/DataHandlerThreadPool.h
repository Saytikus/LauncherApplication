...
public:
    ...
    void StartHandlerThreads();
    void SetMaxThreadCount(const int thread_count);

private:
    int max_threads_count_ = 12;
    int roundrobin_count_ = 0;
    QVector <QThread*> threads;
    QVector <DataHandler*> handlers;

private slots:
    void SendReadBufferChangeToHandler(const QBuffer *read_buffer, const QString buffer_id);
...
signals:
    ...
    void CompleteAnswerFinished(const QByteArray complete_answer, const int size, const QString buffer_id);
