...
QString QUdpClient::Read() {
    ...
    if(!QString(datagram).isEmpty())
        emit PocketReceived(QString(datagram));
    return QString(datagram);
}

QString QUdpClient::HandShakeStart() {
    synchronization_digit_ = QRandomGenerator::global()->bounded(1, 200);
    QString connect_string = QString::number(synchronization_digit_) +
...
QString QUdpClient::HandShakeComplete() {
    ...
    if(AnswerCheck(answer_list, synchronization_digit_)) {
...
void QUdpClient::RegistrationRequestSend(const QString login, const QString password) {
    this->Send("reg_start|" + login + "|" + password + "|reg_end" , server_address_, work_port_);
}

void QUdpClient::AuthorizationRequestSend(const QString login, const QString password) {
...
void QUdpClient::RedirectMessage(const QString message) {
    ...
    if(check_message.size() == 3 &&
       check_message[0] == "reg_answer_start" && check_message[2] == "reg_answer_end") {
        emit RegistrationAnswerReceived(check_message[1].toInt());
    }

    if(check_message.size() == 3 &&
       check_message[0] == "auth_answer_start" && check_message[2] == "auth_answer_end") {
        emit AuthorizationAnswerReceived(check_message[1].toInt());
    }
}

void QUdpClient::ConnectionClosedSend() {
